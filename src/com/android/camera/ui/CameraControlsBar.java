...
public class CameraControlsBar extends FrameLayout implements Rotatable {
    ...
    public void setTransparency(boolean transparent) {
        setBackgroundColor(getResources().getColor(transparent ?
                R.color.camera_control_bg_transparent :
                R.color.camera_controls_bar_bg_opaque));
    }

    protected void setCameraSettingsVisibility(final boolean visible, boolean animate) {
        if (mAnimating) return;
        if (!animate) {
            mCameraControlsSettingsParent.getLayoutParams().height = mDefaultHeight;
    ...
    protected boolean isCameraControlsSettingsVisible() {
        return mCameraControlsSettingsParent.getLayoutParams().height == mDefaultHeight;
    }
